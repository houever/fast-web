    ...
    >
      <Form ref="form" :model="form" :label-width="95" :rules="formValidate">
        ...
        <FormItem label="前端表单路由" prop="routeName">
          <Select v-model="form.routeName" placeholder="输入关联前端表单路由名" clearable>
            <Option v-for="(item, i) in dictFormRoute" :key="i" :value="item.val" :label="item.title">
              <span style="margin-right:10px;">{{ item.title }}</span>
              <span style="color:#ccc;">{{ item.val }}</span>
            </Option>
          </Select>
        </FormItem>
        <FormItem label="关联业务表" prop="businessTable">
          <Select v-model="form.businessTable" placeholder="请选择关联数据库业务表" clearable>
            <Option v-for="(item, i) in dictTable" :key="i" :value="item.val" :label="item.title">
              <span style="margin-right:10px;">{{ item.title }}</span>
              <span style="color:#ccc;">{{ item.val }}</span>
            </Option>
          </Select>
...
<script>
  import {
    ...
    convertToModel,
    deleteProcess
  } from '@/api/activiti/process-manage'
  import {
    initActCategory,
    loadActCategory,
    searchActCategory
  } from '@/api/activiti/category-manage'
  ...
  export default {
    ...
    methods: {
      ...
      getDictDataType() {
        getDictDataByType('business_form_route').then(res => {
          if (res.success) {
            this.dictFormRoute = res.data
          }
        })
      ...
      showResource(type, v) {
        if (type == 0) {
          window.open(`${exportResource}?id=${v.id}&type=${type}`)
        } else if (type == 1) {
          this.viewTitle = '流程图片预览(' + v.diagramName + ')'
          this.diagramUrl = `${exportResource}?id=${v.id}&type=${type}`
          this.viewImage = true
        }
      },
      remove(v) {
